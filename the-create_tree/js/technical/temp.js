...
function setupTempData(layerData, tmpData, funcsData) {
	for (item in layerData){
		...
		else if(typeof layerData[item]==="string"){//我不确定这行不行
			if(layerData[item][0]=='$'){
			try {
				if(Decimal.Reg.test(layerData[item].slice(1))){
					tmpData[item] = new Decimal(layerData[item].slice(1));
				}
				else{
					const funcstr=Decimal.infixToChainString(layerData[item].slice(1),valMap)
					const funct = new Function("x","const _=Decimal.fromComponents_noNormalize;\nreturn "+funcstr);
					if(!activeFunctions.includes(item)){
						funcsData[item] = funct;
						tmpData[item] = decimalOne;
					}
					else{
						tmpData[item] =funcsData[item] =layerData[item]=funct;
					}
				}
			} catch (error) {
			tmpData[item] = layerData[item]
...
function updateTempData(layerData, tmpData, funcsData, useThis) {
	for (item in funcsData){
		...
		else if(isFunction(funcsData[item])&&!isFunction(tmpData[item])){
			Vue.set(tmpData, item,funcsData[item]())
		}
	}	
}
